	...
	*/
    ...
    function is_cacheable( $view_id ) {
		...
		*/
		...
		/**
		 * List of shortcodes than, when placed inside a View layout, should disable the caching mechanism.
		 *
		 * For now, we register the dynamic elements from Views.
		 * Forms should also register its own shortcodes, at least until
		 * form submission is managed on form printing.
		 *
		 * @param array
		 * @return array
		 * @since 2.9.4
		...
		foreach ( $shortcodes_to_shortcircuit_cache as $shortcode_candidate ) {
			if ( strpos( $view_layout_html, '[' . $shortcode_candidate ) !== false ) {
				return false;
			}
		}
	...
	if ( $plugin_file == $this_plugin ) {
	    ...
	    if( wpv_is_views_lite() ){
		    $plugin_meta[] = sprintf(
			    ...
			    'https://wpml.org/version/views-lite-2-9-4/',
			    __( 'Views Lite 2.9.4 release notes', 'wpv-views' )
		    );
        } else {
		    $plugin_meta[] = sprintf(
			    '<a href="%s" target="_blank">%s</a>',
			    'https://toolset.com/version/views-2-9-4/?utm_source=viewsplugin&utm_campaign=views&utm_medium=release-notes-plugin-row&utm_term=Views 2.9.4 release notes',
			    __( 'Views 2.9.4 release notes', 'wpv-views' )
		    );
        }
